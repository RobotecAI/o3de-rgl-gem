...
namespace RGL
{
    namespace Utils
    {
            ...
            rgl_mesh_t& targetMesh, const rgl_vec3f* vertices, size_t vertexCount, const rgl_vec3i* indices, size_t indexCount)
        {
            ...
            {
                ErrorCheck(rgl_mesh_destroy(targetMesh));
                targetMesh = nullptr;
            }
        }
        ...
        {
            const bool success = ErrorCheck(rgl_entity_create(&targetEntity, nullptr, mesh));
            if (!success && !targetEntity)
            {
                ErrorCheck(rgl_entity_destroy(targetEntity));
